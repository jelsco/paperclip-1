...
export function issueRoutes(
  ...
  router.post("/issues/:id/comments", validate(addIssueCommentSchema), async (req, res) => {
    ...
    const commentAccessDecision = governanceSweepComment
      ? null
      : await assertAgentIssueCommentAllowed(req, res, issue);
    if (!governanceSweepComment && !commentAccessDecision) return;
    ...
    const mentionGrantedPeerAgentCommentOnly =
      ...
      !reopenRequested &&
      !resumeRequested &&
      commentAccessDecision !== null &&
      commentAccessDecision !== false &&
    ...
    const governanceAppendOnlyClosedComment =
      ...
      !resumeRequested &&
      !interruptRequested;
    const appendOnlyClosedComment = mentionGrantedPeerAgentCommentOnly || governanceAppendOnlyClosedComment;
    const effectiveReopenRequested = appendOnlyClosedComment ? false : reopenRequested;
    const effectiveResumeRequested = appendOnlyClosedComment ? false : resumeRequested;
    if (
      isClosed &&
      req.actor.type === "agent" &&
      issue.assigneeAgentId !== null &&
      issue.assigneeAgentId !== req.actor.agentId &&
      !appendOnlyClosedComment
    ) {
      if (!(await assertAgentIssueMutationAllowed(req, res, issue))) return;
    }
